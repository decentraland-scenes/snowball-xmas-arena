//import { NoArgCallBack } from "src/types/types";
...
class ItemAnimationSystem {
  ...
  update(dt: number) {
    ...
    for (let item of this.group.entities) {
      if (!item.hasComponent(AnimatedItem)) return;
      ...
      if (!info.isHighlighted) {
        ...
        //log("selected.this.group.entities",info.isHighlighted,info.text,info.animFraction,transform.scale,(item as Entity).uuid,(item as Entity).name,item.hasComponent(GLTFShape))
