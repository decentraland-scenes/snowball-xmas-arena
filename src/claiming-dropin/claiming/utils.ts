...
export function customResolveSourceImageSize(data:ItemData):number{
    ...
    if(
            testForAddress(data,WearableEnum.PLAY_GAMES)
            ||testForAddress(data,WearableEnum.WINNING_TEAM)
            ||testForAddress(data,WearableEnum.RANK_TOP3)
            ||testForAddress(data,WearableEnum.RANK1)
            ||testForAddress(data,WearableEnum.PLAY_DAYS)
    ){
        sourceSize = 1024 
    }else if(testForWearable(data,WearableEnum.ARTNET_HAT_URN) ){
        sourceSize = 512 
    }
