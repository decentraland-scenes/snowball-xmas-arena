...
export type PlayFabSettings = {
  titleId:string
  developerSecretKey:string
}

function applySettings(settings:PlayFabSettings){
  PlayFab.settings.titleId = settings.titleId
  PlayFab.settings.developerSecretKey = settings.developerSecretKey
}
export const GetEntityToken = (settings:PlayFabSettings,request:PlayFabAuthenticationModels.GetEntityTokenRequest):Promise<PlayFabAuthenticationModels.ValidateEntityTokenResponse> => {
...
export const AddCharacterVirtualCurrency = (settings:PlayFabSettings,request:PlayFabServerModels.AddUserVirtualCurrencyRequest):Promise<PlayFabServerModels.ModifyUserVirtualCurrencyResult> => {
    return new Promise((resolve, reject)=>{
      applySettings(settings)
...
export const SubtractUserVirtualCurrency = (settings:PlayFabSettings,request:PlayFabServerModels.SubtractUserVirtualCurrencyRequest):Promise<PlayFabServerModels.ModifyUserVirtualCurrencyResult> => {
  return new Promise((resolve, reject)=>{
    applySettings(settings)
...
export const AddUserVirtualCurrency = (settings:PlayFabSettings,request:PlayFabServerModels.AddUserVirtualCurrencyRequest):Promise<PlayFabServerModels.ModifyUserVirtualCurrencyResult> => {
  return new Promise((resolve, reject)=>{
    applySettings(settings)
    PlayFabServer.AddUserVirtualCurrency( request, c(resolve, reject)) 
  })
...
export const GetPlayerCombinedInfo = (settings:PlayFabSettings,request:PlayFabServerModels.GetPlayerCombinedInfoRequest):Promise<PlayFabServerModels.GetPlayerCombinedInfoResult> => {
  return new Promise((resolve, reject)=>{
    applySettings(settings)
...
export const UpdateUserReadOnlyData = (settings:PlayFabSettings,request:PlayFabServerModels.UpdateUserDataRequest):Promise<PlayFabServerModels.UpdateUserDataResult> => {
  return new Promise((resolve, reject)=>{
    applySettings(settings)
...
export const UpdatePlayerStatistics = (settings:PlayFabSettings,request:PlayFabServerModels.UpdatePlayerStatisticsRequest):Promise<PlayFabServerModels.UpdatePlayerStatisticsResult> => {
  return new Promise((resolve, reject)=>{
    applySettings(settings)
...
export const AuthenticateSessionTicket = (settings:PlayFabSettings,request:PlayFabServerModels.AuthenticateSessionTicketRequest):Promise<PlayFabServerModels.AuthenticateSessionTicketResult> => {
  return new Promise((resolve, reject)=>{
    try{
      applySettings(settings)
...
export const EndLevelGivePlayerUpdatePlayerStats = async (settings:PlayFabSettings,updateStats:EndLevelUpdatePlayerStatsRequest):Promise<EndLevelUpdatePlayerStatsResult> => {
    ...
    GetPlayerCombinedInfo(settings,getPlayerCombinedInfoRequest).then(
      function(result:PlayFabServerModels.GetPlayerCombinedInfoResult){
        ...
        */
        ...
        //must write it
        const updateReadOnlyData = UpdateUserReadOnlyData(settings,
          {
            // The optional custom tags associated with the request (e.g. build number, external trace identifiers, etc.).
            //CustomTags?: { [key: string]: string | null };
        ...
        if(updateStats.place > 0 && Math.abs(updateStats.totalTime) < Math.abs(CONFIG.MAX_POSSIBLE_RACE_TIME)){
          ...
          addStat(thisGameStats,"totalTime_best",prefix,["hour","day","week","epoch"],updateStats.totalTime*workaroundScalar,[CONFIG.MAX_POSSIBLE_RACE_TIME*workaroundScalar + 1,1*workaroundScalar])
          addStat(thisGameStats,"lapTime_best",prefix,["hour","day","week","epoch"],Math.min.apply(Math,updateStats.lapTimes)*workaroundScalar,[CONFIG.MAX_POSSIBLE_RACE_TIME*workaroundScalar + 1,1*workaroundScalar])
          ...
          //addStat(thisGameStats,"trk_feat_destroyed","level_any",["hour","day","week","epoch"],Math.min.apply(Math,updateStats.lapTimes)*workaroundScalar,[CONFIG.MAX_POSSIBLE_RACE_TIME*workaroundScalar + 1,1*workaroundScalar])
          //addStat(thisGameStats,"projectiles_fired","level_any",["hour","day","week","epoch"],Math.min.apply(Math,updateStats.lapTimes)*workaroundScalar,[CONFIG.MAX_POSSIBLE_RACE_TIME*workaroundScalar + 1,1*workaroundScalar])
        ...
        const updatePlayerStats = UpdatePlayerStatistics(settings,
          {
            // The optional custom tags associated with the request (e.g. build number, external trace identifiers, etc.).
            //CustomTags?: { [key: string]: string | null };
...
export function addStat(thisGameStats:PlayFabServerModels.StatisticUpdate[],stat: string, prefix:string, statFrequency: string[], value: number,range:number[]) {
  if(value === undefined || Number.isNaN(value) || value < range[0] || value > range[1]){
    console.log("addStat ",stat," out of range, not recording ",value,range);
    return
  }
  
  for(const p in statFrequency){
    if(statFrequency[p].charAt(0) == 'x' || statFrequency[p].charAt(0) == 'X'){
      console.log("addStat ",stat,statFrequency[p]," commented out ignoring ",value,range);
      continue;
    }
